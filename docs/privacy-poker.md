...
FHEVM provides several encrypted types:

- `ebool`: Encrypted boolean values
- `euint8`, `euint16`, `euint32`, `euint64`: Encrypted unsigned integers
- Arrays of encrypted types for complex data structures

### Access Control

Two essential operations for managing encrypted data:

```solidity
// Grant permission to the contract itself
FHE.allowThis(encryptedValue);

// Grant permission to a specific address
FHE.allow(encryptedValue, userAddress);
```

{% hint style="warning" %}
**Important**: Always grant both `allowThis` and `allow` permissions when storing encrypted values that need to be accessed by both the contract and users.
{% endhint %}
...
{% tab title="PokerGame.test.ts" %}

See the test file for comprehensive examples showing:
...
2. Game creation with validation
3. Player joining and game start
4. Player moves (call, raise, fold)
5. Card revealing mechanisms
6. Access control enforcement
7. FHE-specific patterns
...
function badExample(euint32 value) external {
    _count = value;
    FHE.allow(_count, msg.sender); // Missing allowThis - contract can't access!
}
```
...
**Problem**: Attempting to decrypt values in view functions

```solidity
// WRONG
function getBet() external view returns (uint32) {
    return _encryptedBet; // Can't decrypt in view!
}
```

**Solution**: Return the encrypted handle
